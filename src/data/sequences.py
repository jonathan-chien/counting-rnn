...
class Hypercube:
    ...
    def _validate(self, num_dims, inclusion_set, coords, encoding):
        ...
        """
        utils.validate_tensor(inclusion_set, 2)
        utils.validate_tensor(coords, 1)
...
class SeqLengths:
    ...
    def __init__(self, *args):
        ...
        self.lengths = dict()
        for arg in args:
            utils.validate_tensor(arg[0], 1)
            utils.validate_pmf(arg[1], len(arg[0]))
            self.lengths[arg[2]] = {
                'support' : arg[0],
                'pmf' : arg[1]
            }
...
class Sequences(Dataset):
    ...
    def sample(self):
        ...
        def get_row_label(A, B):
            ...
            """
            utils.validate_tensor(A, 2)
            utils.validate_tensor(B, 2)
        ...
        # # Get labels.
        # if self.seq_order == 'pos_first' or self.seq_order == 'permute':
        #     class_1_label = torch.tensor([-1], dtype=self.dtype)
        #     class_2_label = class_1_label - 1
        # else:
        #     class_1_label = torch.tensor([-2], dtype=self.dtype)
        #     class_2_label = class_1_label + 1
        
        # self.labels = [
        #     torch.cat(
        #         (torch.tile(class_1_label, (self.seq_lens[class_1][i_seq],)),
        #          torch.tile(class_2_label, (self.seq_lens[class_2][i_seq],)))
        #     )
        #     for i_seq in range(self.num_seq)
        # ]
...
class EmbedData:
    ...
    def __init__(
            ...
            noise=None # Should be a dictionary containing distribution method and its parameters, e.g. {'distr': torch.Tensor.normal_, 'distr_params' : {'mean' : 0, 'std' : 0.1}}
        ):
    ...
    def _noise(self, x):
        """ 
        E.g., distr_method = torch.Tensor.normal_.
        """
        distr_method = self.noise['distr']
        noise = distr_method(torch.full(x.shape, torch.nan), **self.noise['distr_params'])
        return x + noise
    ...
    def __call__(self, data):
        ...
        if self.lin_transform is not None: 
            transformed = data_padded @ self.lin_transform
        else:
            transformed = data_padded

        if self.noise is not None: transformed = self._noise(transformed)
